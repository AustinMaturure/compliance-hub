...
export function PracticeSection() {
  ...
  {services.map((service, i) => (
    <div
      key={service.title}
      className="motion-reveal motion-reveal-card lift-card flex h-full flex-col rounded-xl border border-(--border) bg-(--surface-elevated) p-6"
      style={{ '--motion-delay': `${i * 60}ms` } as React.CSSProperties}
    >
      <h2 className="cabinet text-2xl font-bold mb-2 leading-tight text-(--text-h) md:mb-3 md:min-h-[64px]">
        {service.title}
      </h2>
