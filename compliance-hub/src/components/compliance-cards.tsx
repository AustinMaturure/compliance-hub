    ...
    <section className="w-full">
      <div className="w-full border-y border-neutral-300">
        {complianceCards.map((card, index) => (
          <article
            key={card.title}
            className={`motion-reveal grid gap-4 border-b border-neutral-300 py-9 last:border-b-0 md:grid-cols-[32px_minmax(220px,1fr)_minmax(300px,360px)] md:items-start md:gap-12 ${
              index === 1 ? 'motion-delay-1' : index >= 2 ? 'motion-delay-2' : ''
            }`}
          >
            <p className="pt-1 text-base font-medium">{index + 1}</p>
            <h3 className="text-[2rem] font-medium leading-[1.05] tracking-tight">{card.title.toLowerCase()},</h3>
